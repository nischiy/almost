#!/usr/bin/env python3
# scripts/diagnostics/trade_once.py  (path-robust, one-shot trade via adapters)
...
if importlib.util.find_spec("app.services.notifications"):
    from app.services.notifications import place_order_via_rest
else:
...
def main():
    ...
    if wallet is None:
        print(json.dumps({"error": "Set WALLET_USDT env."}, indent=2)); return
