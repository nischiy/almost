...
_HEALTH_DIR = PROJECT_ROOT / "logs" / "health" / _dt.date.today().isoformat()
_HEALTH_DIR.mkdir(parents=True, exist_ok=True)
_HEALTH_LOG = _HEALTH_DIR / "health.jsonl"
...
def kill(reason: str, context: Optional[Dict[str, Any]] = None) -> None:
    ...
    try:
        FLAG_FILE.write_text(reason, encoding="utf-8")
    except Exception:
...
def compute_sl_tp(entry_price: float, side: str, atr: Optional[float]) -> tuple[Optional[float], Optional[float]]:
    mode = os.getenv("SL_MODE", "atr")  # 'atr' or 'none' or 'percent'
